...
$sql=sprintf("select * from table_info where tablenum=%d",$tablenum);
...
$sql=sprintf("select * from order_detail where order_id in (select id from table_order where table_id=%d)",$tableID);
$rs=$db->query($sql) or die("查询失败！");
$historyTotalPrice=0;
...
while($row=$rs->fetchArray())
{
	...
	$row1=$rs1->fetchArray();
	$historyTotalPrice+=$row1['price']*$row['quantity'];
echo $row1['name']."(". $row['quantity'].")<br/>";
}
